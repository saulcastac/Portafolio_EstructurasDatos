...
template<class K, class T>
void HashTable<K, T>::reSize(int newSize){
  maxSize = newSize;
  size = 0;
  table = std::vector<HashNode<K,T>>(maxSize);
}
...
void HashTable<K,T>::print() {
  ...
  for (int i = 0; i < maxSize; i++) {
    ...
    if (table[i].getStatus() == 1) // Cell is used
      std::cout << "Cell: " << i << ", Key: " << table[i].getKey() << ", Value: " << table[i].getData() << ", Colisiones: " << table[i].getOverflowSize() << std::endl;
  }
}

template<class K, class T>
void HashTable<K,T>::overflowSize(){
  int contador_total = 0; 
  //std::cout << "Numero de colisiones por key" << std::endl;
  for(int i = 0; i < maxSize; i++){
   if(table[i].getStatus() == 1){
     //std::cout << "Cell: " << i << " Key: " << table[i].getKey() << ", Num. Colisiones: " << table[i].getOverflowSize() << std::endl;
     if (table[i].getOverflowSize() > 0){
       contador_total++;
     }
   } 
  }
  std::cout<< "El número total de colisiones es de: " << contador_total << std::endl;
}
